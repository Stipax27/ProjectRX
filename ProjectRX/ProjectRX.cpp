    ...
    if (choice > 0 && choice <= shuffledChoices.size()) {
        if (availableChoices[choice - 1]) {
            ...
            if (currentRound == 8 && selected.persuasion == 12 && selected.nextRound == 10) {
                ...
                cout << "\n\t\033[34m=== СОЛОВЕЙ ===\033[0m\n";
                cout << endl;
                cout << "(Делает шаг назад) Так тому и быть, я покажу тебе, где раки зимуют!";
                provokedFight = true;
            }
...
void ShowEnding() {
    ...
    if (provokedFight) {
        ...
        cout << "Соловей (протягивает сундук): Черт возьми, парень. В тебе есть сталь. ";
...
void PlayGame() {
    ...
    rounds.push_back({
        ...
        },
        {
            ...
            {"\"[Отдать мешок с золотом] Такое, что делает твою жизнь короче, а мои предложения — привлекательнее. Не заглядывай в пасть к дракону, просто продай яйцо.\"", 8, 9}, // При наличии золота ведет к успеху
    ...
    rounds.push_back({
        ...
        {
            {"\"[ Напасть! ]Попробуй, гниль лесная. И узнаешь, почему меня в деревне самым безбашенным считают. Этот сундук — единственное, что стоит между тобой и очень болезненным знакомством моего кулака.\"", 12, 10}, // Провоцирует бой
        ...
        if (currentRound == 8 && hasGold) {
            ...
            for (const auto& choice : rounds[7].choices) {
                if (choice.persuasion == 8 && choice.nextRound == 9) {
                    ...
                    cout << "\n\t\033[34m=== СОЛОВЕЙ ===\033[0m\n";
                    cout << endl;
                    cout << "(Подходит вплотную и забирает мешок, пересчитывает) Как же ты меня доколебал с этим сундуком.Но звонкая монета это аргумент.\n", // Оптимальный (золото)";
